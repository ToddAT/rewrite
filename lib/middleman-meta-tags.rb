...
class MetaTags < Middleman::Extension
	...
	helpers do
		...
		def concat(site_data, name, key, default = '')
			value = ''

			(value << site_data[key]) unless site_data[key].blank?
			(value << ", ") unless site_data[key].blank?
			(value << self.meta.tags[key]) unless self.meta.tags[key].blank?
